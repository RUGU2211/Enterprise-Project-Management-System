...
public class ProjectTaskService {
    ...
    public Iterable<ProjectTask> findBacklogById(String backlog_id, String username){
        // Find the project and ensure the user has access to it
        Project project = projectService.findProjectByIdentifier(backlog_id, username);
        
        // Get the backlog - create one if it doesn't exist
        Backlog backlog = project.getBacklog();
        if (backlog == null) {
            System.out.println("Backlog not found for project " + backlog_id + ", creating one now");
            backlog = new Backlog();
            backlog.setProject(project);
            backlog.setProjectIdentifier(backlog_id.toUpperCase());
